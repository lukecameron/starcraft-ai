...
The immediate blockers are dependency completeness and API alignment: the pinned checkout contains a full `3rdparty/openbw/bwapi` tree plus vendored BWEM/FAP and serialization/compression libraries, but its non-MSVC branch builds that bundled OpenBW/BWAPI instead of consuming this project’s `third_party/bwapi` at `48124ba8ed1b4d52b3dfd52acbaf34afb9a37fe2`. There is no `.gitmodules` indirection at this revision, so the source is self-contained; the risk is duplicate engine ABI/API rather than missing submodules. Stardust relies on `getRemainingLatencyFrames`, `getGroundHeight`, static-neutral enumeration, event access, and BWEM’s `bwapiExt.h`; these are present in the local 4.2-derived API surface, but must be compiled against the exact OpenBW headers before reuse. Its `test` target is the native integration path and should be adapted to emit the project’s module symbols. First test should therefore be a clean configure of Stardust’s own non-MSVC target with the pinned commit, then a narrow adapter smoke match against WorkerRush on Benzene. Capture compiler, CMake, bundled dependency, and engine hashes; a successful compile is not an opponent result.
