...
All 20 replay files were copied successfully with SHA-256 hashes and sizes in their match manifests, totaling 13,610,191 bytes. The existing `.tools/screp/screp` parser returned zero for all 20 files: each reported the expected map and both expected player names/races, and each replay header’s `Frames + 1` matched the owning player’s recorded final frame count. This is parser/header evidence only; the launcher did not perform replay playback validation, so it does not establish semantic playback validity. Raw parser summaries are preserved in `artifacts/experiments/quick-mcrave-as-bounds-v1/screp-parser-summary.json`. The authoritative aggregate ledger is `artifacts/experiments/quick-mcrave-as-bounds-v1/manifest.json`; its ten `match_manifest` entries point to the raw logs, metadata and replay records. It remains unchanged.
