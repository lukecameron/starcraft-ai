...
This two-site change is sufficient to test the demonstrated frame-3 cause. It is not a claim that all later McRave behavior becomes deterministic: the codebase contains other default `set<shared_ptr<...>>` collections and may contain address-dependent or opponent-dependent ordering. A follow-up should first require identical normalized commands through a fixed early frame; if another divergence appears, it should be traced before extending the stabilization scope.
