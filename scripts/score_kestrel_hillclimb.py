...
def _candidate_generation(candidate_name: object) -> str | None:
    ...
    if not isinstance(candidate_name, str):
        return None
    match = re.search(r"(?:^|[^a-z0-9])v(45|44|43|42|41|40|38|37|36)(?:[^a-z0-9]|$)", candidate_name.lower())
...
def _diagnostic_generation(metadata: dict[str, object], candidate_name: object = None) -> str:
    ...
    if candidate_generation in {"v36", "v37", "v38", "v40", "v41", "v42", "v43", "v44", "v45"}:
        return candidate_generation
    if any(name in metadata for name in (
        "zerg_four_probe_policy_active_frames",
        "zerg_four_probe_pylon_accepted_frame",
        "zerg_four_probe_pylon_probe_count",
    )):
        return "v45"
    if any(name in metadata for name in (
        "zerg_sixth_probe_restoration_active_frames",
        "zerg_sixth_probe_accepted_frame",
        "zerg_sixth_probe_accepted_count",
        "zerg_post_pylon_pre_second_gateway_probe_train_count",
    )):
        return "v44"
    if any(name in metadata for name in (
...
def _construction_pending_summary(metadata: dict[str, object], opponent_race: str | None = None,
    ...
    return {
        "generation": generation if generation in {"v35", "v36", "v37", "v38", "v40", "v41", "v42", "v43", "v44", "v45"} else "v35",
...
def _v43_treatment_summary(metadata: dict[str, object], opponent_race: str | None = None,
                            require_ordering: bool = False) -> dict[str, object]:
    ...
    ordering_name = "zerg_second_zealot_probe_reserve_block_pre_accept_flags"
    required_names = names + (ordering_name,) if require_ordering else names
    ...
    block_pre_accept_flags = metadata.get(ordering_name)
    ...
    if ordering_name in metadata and not isinstance(block_pre_accept_flags, list):
        invalid.append(ordering_name)
        flags.append("zerg_second_zealot_probe_reserve_block_pre_accept_flags_type_mismatch")
    flags.extend(f"{name}_missing" for name in missing)
    race = _normalise_race(opponent_race)
    if race is None:
        known_zerg = metadata.get("known_zerg")
    ...
    pre_accept_values = block_pre_accept_flags if isinstance(block_pre_accept_flags, list) else []
    if len(frame_values) != len(mineral_values):
        flags.append("reserve_trace_length_mismatch")
    if require_ordering and len(frame_values) != len(pre_accept_values):
        flags.append("reserve_ordering_trace_length_mismatch")
    ...
    if any(not isinstance(value, int) or isinstance(value, bool) or value not in {0, 1} for value in pre_accept_values):
        flags.append("reserve_block_pre_accept_flag_invalid")
    ...
    if race == "zerg":
        ...
        effective_frames = [frame for frame, flag in zip(frame_values, pre_accept_values)
                            if flag == 1] if require_ordering else frame_values
        if isinstance(second_frame, int) and any(isinstance(frame, int) and frame >= second_frame for frame in effective_frames):
            flags.append("reserve_block_after_second_zealot")
        if require_ordering and isinstance(second_frame, int):
            if any(flag == 0 and frame != second_frame for frame, flag in zip(frame_values, pre_accept_values)):
                flags.append("reserve_block_provisional_frame_mismatch")
        if reserve != 100:
    ...
    checks = {
        ...
        "reserve_ordering_trace_aligned": not require_ordering or "reserve_ordering_trace_length_mismatch" not in flags,
    ...
    return {
        ...
        "reserve_block_pre_accept_flags": pre_accept_values,
...
def _v44_treatment_summary(metadata: dict[str, object], opponent_race: str | None = None) -> dict[str, object]:
    """Validate v44's bounded sixth-Probe restoration telemetry."""
    names = (
        "zerg_sixth_probe_restoration_active_frames",
        "zerg_sixth_probe_accepted_frame",
        "zerg_sixth_probe_accepted_count",
    ...
    if race is None:
        ...
        if isinstance(known_zerg, bool):
            race = "zerg" if known_zerg else "non-zerg"
    active = values[names[0]]
    accepted_frame = values[names[1]]
    accepted_count = values[names[2]]
    pre_second_gateway_count = values[names[3]]
    first_pylon = metadata.get("first_pylon_accepted_frame")
    second_gateway = metadata.get("second_gateway_current_frame")
    if race == "zerg":
        if not isinstance(active, int) or isinstance(active, bool) or active <= 0:
            flags.append("zerg_sixth_probe_restoration_active_frames_not_positive")
        if accepted_count != 1:
            flags.append("zerg_sixth_probe_accepted_count_not_one")
        if not isinstance(accepted_frame, int) or isinstance(accepted_frame, bool) or accepted_frame < 0:
            flags.append("zerg_sixth_probe_accepted_frame_not_observed")
        if pre_second_gateway_count != 1:
            flags.append("zerg_pre_second_gateway_probe_train_count_not_one")
        if isinstance(first_pylon, int) and isinstance(accepted_frame, int) and accepted_frame < first_pylon:
            flags.append("zerg_sixth_probe_accepted_before_first_pylon")
        if isinstance(second_gateway, int) and isinstance(accepted_frame, int) and accepted_frame >= second_gateway:
            flags.append("zerg_sixth_probe_accepted_at_or_after_second_gateway")
    elif race in {"terran", "protoss", "random", "non-zerg"}:
        if active != 0:
            flags.append("non_zerg_sixth_probe_restoration_active_frames_nonzero")
        if accepted_frame != -1 or accepted_count != 0 or pre_second_gateway_count != 0:
            flags.append("non_zerg_sixth_probe_restoration_activity")
    else:
        flags.append("sixth_probe_restoration_race_unknown")
    checks = {
        "active_frames_positive": race != "zerg" or "zerg_sixth_probe_restoration_active_frames_not_positive" not in flags,
        "sixth_probe_accepted_once": race != "zerg" or "zerg_sixth_probe_accepted_count_not_one" not in flags,
        "sixth_probe_frame_ordered": race != "zerg" or not any(flag.startswith("zerg_sixth_probe_accepted_") and flag != "zerg_sixth_probe_accepted_count_not_one" for flag in flags),
        "no_seventh_pre_second_gateway_probe": race != "zerg" or "zerg_pre_second_gateway_probe_train_count_not_one" not in flags,
        "non_zerg_inactive": race not in {"terran", "protoss", "random", "non-zerg"} or not any(
            flag.startswith("non_zerg_sixth_probe_restoration") for flag in flags),
    }
    return {
        "generation": "v44",
        "telemetry_status": "complete" if not missing and not invalid else "partial",
        ...
        "invalid_fields": sorted(set(invalid)),
        "active_frames": active if isinstance(active, int) and not isinstance(active, bool) else None,
        "accepted_frame": accepted_frame if isinstance(accepted_frame, int) and not isinstance(accepted_frame, bool) else None,
        "accepted_count": accepted_count if isinstance(accepted_count, int) and not isinstance(accepted_count, bool) else None,
        "pre_second_gateway_probe_train_count": pre_second_gateway_count if isinstance(pre_second_gateway_count, int) and not isinstance(pre_second_gateway_count, bool) else None,
        "checks": checks,
        "quantitative_grade": {"score": round(100 * sum(value for value in checks.values()) / len(checks)),
                               "max_score": 100, "grade": "pass" if not flags else "review"},
        "review_flags": sorted(set(flags)),
...
def _v45_treatment_summary(metadata: dict[str, object], opponent_race: str | None = None) -> dict[str, object]:
    """Validate v45's four-Probe opening and direct completion telemetry."""
    names = (
        "zerg_four_probe_policy_active_frames",
    ...
    required_names = names + ("first_pylon_accepted_frame",)
    present = [name for name in required_names if name in metadata]
    missing = [name for name in required_names if name not in metadata]
    invalid: list[str] = []
    flags: list[str] = []
    scalar_names = names[:6]
    for name in scalar_names:
        value = metadata.get(name)
        expected_bool = name == "zerg_four_probe_pylon_accepted"
        if name in metadata and (not isinstance(value, bool) if expected_bool else
                                  not isinstance(value, int) or isinstance(value, bool)):
            invalid.append(name)
            flags.append(f"{name}_type_mismatch")
    ordering_name = names[6]
    ordering_values = metadata.get(ordering_name)
    if ordering_name in metadata and not isinstance(ordering_values, list):
        invalid.append(ordering_name)
        flags.append(f"{ordering_name}_type_mismatch")
    flags.extend(f"{name}_missing" for name in missing)
    race = _normalise_race(opponent_race)
    if race is None:
        known_zerg = metadata.get("known_zerg")
    ...
    accepted = metadata.get(names[1])
    accepted_frame = metadata.get(names[2])
    pylon_probe_count = metadata.get(names[3])
    second_train = metadata.get(names[4])
    second_completed = metadata.get(names[5])
    accepted_probes = _metadata_list(metadata, "accepted_probe_train_frames")
    first_pylon = metadata.get("first_pylon_accepted_frame")
    if "first_pylon_accepted_frame" in metadata and (not isinstance(first_pylon, int) or isinstance(first_pylon, bool)):
        invalid.append("first_pylon_accepted_frame")
        flags.append("first_pylon_accepted_frame_type_mismatch")
    second_gateway = metadata.get("second_gateway_current_frame")
    pre_accept_values = ordering_values if isinstance(ordering_values, list) else []
    if any(not isinstance(value, int) or isinstance(value, bool) or value not in {0, 1}
           for value in pre_accept_values):
        flags.append(f"{ordering_name}_value_type_mismatch")
    if race == "zerg":
        if not isinstance(active, int) or isinstance(active, bool) or active <= 0:
            flags.append("zerg_four_probe_policy_active_frames_not_positive")
        if accepted is not True:
            flags.append("zerg_four_probe_pylon_accepted_not_true")
        if not isinstance(accepted_frame, int) or isinstance(accepted_frame, bool) or accepted_frame < 0:
            flags.append("zerg_four_probe_pylon_accepted_frame_not_observed")
        if isinstance(first_pylon, int) and isinstance(accepted_frame, int) and accepted_frame != first_pylon:
            flags.append("zerg_four_probe_pylon_accepted_frame_mismatch")
        if pylon_probe_count != 4:
            flags.append("zerg_four_probe_pylon_probe_count_not_four")
        if not isinstance(second_train, int) or isinstance(second_train, bool) or second_train < 0:
            flags.append("zerg_second_zealot_train_frame_not_observed")
        if not isinstance(second_completed, int) or isinstance(second_completed, bool) or second_completed < 0:
            flags.append("zerg_second_zealot_completed_frame_not_observed")
        if isinstance(second_train, int) and isinstance(second_completed, int) and second_completed <= second_train:
            flags.append("zerg_second_zealot_completed_before_train")
        if isinstance(first_pylon, int) and any(isinstance(frame, int) and frame <= first_pylon for frame in accepted_probes):
            flags.append("zerg_pre_pylon_probe_train_accepted")
        if isinstance(second_gateway, int) and isinstance(second_train, int) and any(
            isinstance(frame, int) and second_gateway < frame < second_train for frame in accepted_probes
        ):
            flags.append("zerg_probe_train_strictly_between_second_gateway_and_second_zealot")
    elif race in {"terran", "protoss", "random", "non-zerg"}:
        if active != 0:
            flags.append("non_zerg_four_probe_policy_active_frames_nonzero")
        if accepted is not False:
            flags.append("non_zerg_four_probe_pylon_accepted_not_false")
        if accepted_frame != -1 or pylon_probe_count != -1:
            flags.append("non_zerg_four_probe_pylon_acceptance_activity")
        if accepted_probes:
            flags.append("non_zerg_four_probe_train_activity")
        if pre_accept_values:
            flags.append("non_zerg_four_probe_ordering_activity")
    else:
        flags.append("four_probe_opening_race_unknown")
    checks = {
        "four_probe_policy_active": race != "zerg" or "zerg_four_probe_policy_active_frames_not_positive" not in flags,
        "four_probe_pylon_accepted": race != "zerg" or not any(flag.startswith("zerg_four_probe_pylon_accepted") for flag in flags),
        "four_probe_pylon_exact_count": race != "zerg" or "zerg_four_probe_pylon_probe_count_not_four" not in flags,
        "zero_pre_pylon_probe_acceptance": race != "zerg" or "zerg_pre_pylon_probe_train_accepted" not in flags,
        "second_zealot_completion_ordered": race != "zerg" or not any(flag.startswith("zerg_second_zealot_") for flag in flags),
        "no_strict_second_gateway_to_second_zealot_probe": race != "zerg" or "zerg_probe_train_strictly_between_second_gateway_and_second_zealot" not in flags,
        "non_zerg_inactive": race not in {"terran", "protoss", "random", "non-zerg"} or not any(
            flag.startswith("non_zerg_four_probe") for flag in flags),
    }
    return {
        "generation": "v45",
        "telemetry_status": "complete" if not missing and not invalid else "partial",
        ...
        "pylon_accepted": accepted if isinstance(accepted, bool) else None,
        "pylon_accepted_frame": accepted_frame if isinstance(accepted_frame, int) and not isinstance(accepted_frame, bool) else None,
        "pylon_probe_count": pylon_probe_count if isinstance(pylon_probe_count, int) and not isinstance(pylon_probe_count, bool) else None,
        "second_zealot_train_frame": second_train if isinstance(second_train, int) and not isinstance(second_train, bool) else None,
        "second_zealot_completed_frame": second_completed if isinstance(second_completed, int) and not isinstance(second_completed, bool) else None,
        "pre_pylon_probe_train_frames": [frame for frame in accepted_probes if isinstance(frame, int) and isinstance(first_pylon, int) and frame <= first_pylon],
        "checks": checks,
        "quantitative_grade": {"score": round(100 * sum(value for value in checks.values()) / len(checks)),
                               "max_score": 100, "grade": "pass" if not flags else "review"},
        "review_flags": sorted(set(flags)),
...
def _probe_reserve_summary(metadata: dict[str, object], opponent_race: str | None = None,
    ...
    feature_present = any(name in metadata for name in names) or (
        ...
        generation == "v44" and any(name in metadata for name in v44_names)
    ) or (
        generation == "v45" and any(name in metadata for name in v45_names)
    )
    if not feature_present:
        if generation in {"v36", "v37", "v38", "v40", "v41", "v42", "v43", "v44", "v45"}:
            required_names = (names + (pre_pylon_name,) if generation == "v41" else
                              names + v42_names if generation == "v42" else
                              names + v42_names + v43_names if generation == "v43" else
                              names + v42_names + v43_names + v44_names if generation == "v44" else
                              names + v43_names + v45_names if generation == "v45" else names)
            return {
                ...
                "v42_treatment": _v42_treatment_summary(metadata, opponent_race) if generation in {"v42", "v43", "v44"} else None,
                "v43_treatment": _v43_treatment_summary(metadata, opponent_race, generation in {"v44", "v45"}) if generation in {"v43", "v44", "v45"} else None,
                "v44_treatment": _v44_treatment_summary(metadata, opponent_race) if generation == "v44" else None,
                "v45_treatment": _v45_treatment_summary(metadata, opponent_race) if generation == "v45" else None,
    ...
    if v42_treatment is not None:
        prefix = "v42:" if generation == "v42" else "v43:retained_v42:" if generation == "v43" else "v44:retained_v42:"
        review_flags.extend(f"{prefix}{flag}" for flag in v42_treatment["review_flags"])
    v43_treatment = _v43_treatment_summary(metadata, opponent_race, generation in {"v44", "v45"}) if generation in {"v43", "v44", "v45"} else None
    if v43_treatment is not None:
        prefix = "v43:" if generation == "v43" else "v44:retained_v43:" if generation == "v44" else "v45:retained_v43:"
        review_flags.extend(f"{prefix}{flag}" for flag in v43_treatment["review_flags"])
    v44_treatment = _v44_treatment_summary(metadata, opponent_race) if generation == "v44" else None
    if v44_treatment is not None:
        review_flags.extend(f"v44:{flag}" for flag in v44_treatment["review_flags"])
    v45_treatment = _v45_treatment_summary(metadata, opponent_race) if generation == "v45" else None
    if v45_treatment is not None:
        review_flags.extend(f"v45:{flag}" for flag in v45_treatment["review_flags"])
    ...
    if generation not in {"v44", "v45"} and in_opening_window and any(isinstance(frame, int) and first_pylon <= frame < second_gateway for frame in accepted):
    ...
    if generation in {"v44", "v45"} and second_gateway >= 0 and second_zealot >= 0 and any(
    ...
    if expected_inactive:
        ...
        v44_ok = v44_treatment is None or v44_treatment["quantitative_grade"]["grade"] == "pass"
        v45_ok = v45_treatment is None or v45_treatment["quantitative_grade"]["grade"] == "pass"
        grade = "pass" if checks["non_zerg_inactive"] and v42_ok and v43_ok and v44_ok and v45_ok else "review"
    elif not review_flags:
        grade = "pass" if block_frames else "untested"
    else:
        grade = "review"
    return {
        "generation": generation if generation in {"v36", "v37", "v38", "v40", "v41", "v42", "v43", "v44", "v45"} else "legacy" if not feature_present else "v36",
        ...
        "invalid_fields": sorted(set(invalid_fields + invalid_scalars +
                                      ...
                                      (v44_treatment["invalid_fields"] if v44_treatment else []) +
                                      (v45_treatment["invalid_fields"] if v45_treatment else []))),
        ...
        "v44_treatment": v44_treatment,
        "v45_treatment": v45_treatment,
        "checks": checks,
...
def _emergency_bridge_summary(metadata: dict[str, object], opponent_race: str | None = None,
    ...
    generation = generation or _diagnostic_generation(metadata)
    army_generation = "three" if generation in {"v34", "v35", "v36", "v37", "v38", "v40", "v41", "v42", "v43", "v44", "v45"} else "two"
    ...
    episode_summary = _emergency_episode_summary(metadata, opponent_race, generation) if generation in {"v35", "v36", "v37", "v38", "v40", "v41", "v42", "v43", "v44", "v45"} else None
...
def _zerg_offense_stage_summary(metadata: dict[str, object], opponent_race: str | None = None,
    ...
    if not feature_present:
        ...
        telemetry_status = "complete" if not missing else "partial"
    return {
        "generation": generation if feature_present and generation in {"v34", "v35", "v36", "v37", "v38", "v40", "v41", "v42", "v43", "v44", "v45"} else "v34" if feature_present else "legacy",
        "opponent_race": race,
        "expected_active": expected_active,
        "expected_inactive": expected_inactive,
...
def _scaling_summary(metadata: dict[str, object], generation: str | None = None) -> dict[str, object]:
    ...
    feature_present = generation in {"v37", "v38", "v40", "v41", "v42", "v43", "v44", "v45"} or any(name in metadata for name in required)
    if not feature_present:
        return {
    ...
    if flags:
        ...
        grade = "pass"
    return {
        "generation": generation if generation in {"v37", "v38", "v40", "v41", "v42", "v43", "v44", "v45"} else "v37", "telemetry_status": "complete" if not missing and not invalid else "partial",
...
def _shared_target_summary(metadata: dict[str, object], opponent_race: str | None = None,
    ...
    feature_present = generation in {"v38", "v40", "v41", "v42", "v43", "v44", "v45"} or any(name in metadata for name in required)
    if not feature_present:
        return {
            "generation": "legacy_absent", "telemetry_status": "legacy_absent",
            "fields_present": [], "missing_fields": [], "invalid_fields": [],
    ...
    return {
        "generation": generation if generation in {"v38", "v40", "v41", "v42", "v43", "v44", "v45"} else "v38", "telemetry_status": "complete" if not missing and not invalid else "partial",
        "fields_present": [name for name in required if name in metadata],
...
def _gateway_probe_summary(metadata: dict[str, object], generation: str | None = None) -> dict[str, object]:
    ...
    feature_present = generation in {"v40", "v41", "v42", "v43", "v44", "v45"} or any(name in metadata for name in required)
    if not feature_present:
        return {
            "generation": "legacy_absent", "telemetry_status": "legacy_absent",
            "fields_present": [], "missing_fields": [], "invalid_fields": [],
    ...
    return {
        "generation": generation if generation in {"v40", "v41", "v42", "v43", "v44", "v45"} else "v40",
        "telemetry_status": "complete" if not missing and not invalid else "partial",
        "fields_present": [name for name in required if name in metadata],
        "missing_fields": missing, "invalid_fields": invalid,
        "selections": rows,
...
def _diagnostic_summary(metadata: dict[str, object], root: Path, opponent_race: str | None = None,
    ...
    result: dict[str, object] = {
        ...
        "production": {
            ...
            "second_zealot_completed_frame": metadata.get("second_zealot_completed_frame"),
        },
    }
    result["defense"] = {
...
def _aggregate_gateway_probe(gateway_probe_games: list[dict[str, object]]) -> dict[str, object]:
    ...
    aggregate: dict[str, object] = {
        ...
        "generations": {generation: sum(item.get("generation") == generation for item in gateway_probe_games)
                         for generation in ("v40", "v41", "v42", "v43", "v44", "v45", "legacy_absent")},
...
def main() -> int:
    ...
    episode_games = [
        ...
        if isinstance(bridge.get("episodes"), dict) and (bridge.get("episodes") or {}).get("generation") in {"v35", "v36", "v37", "v38", "v40", "v41", "v42", "v43", "v44", "v45"}
    ]
    ...
    aggregate["zerg_offense_stage"] = {
        ...
        "generations": {generation: sum(stage.get("generation") == generation for stage in stage_games)
                         for generation in ("v34", "v35", "v36", "v37", "v38", "v40", "v41", "v42", "v43", "v44", "v45", "legacy")},
    ...
    aggregate["construction_pending"] = {
        ...
        "generations": {generation: sum(item.get("generation") == generation for item in construction_games)
                         for generation in ("v35", "v36", "v37", "v38", "v40", "v41", "v42", "v43", "v44", "v45", "legacy")},
    ...
    aggregate["probe_reserve"] = {
        ...
        "generations": {generation: sum(item.get("generation") == generation for item in reserve_games)
                         for generation in ("v36", "v37", "v38", "v40", "v41", "v42", "v43", "v44", "v45", "legacy")},
    ...
    aggregate["scaling"] = {
        ...
        "generations": {generation: sum(item.get("generation") == generation for item in scaling_games)
                         for generation in ("v37", "v38", "v40", "v41", "v42", "v43", "v44", "v45", "legacy_absent")},
    ...
    aggregate["shared_target"] = {
        ...
        "generations": {generation: sum(item.get("generation") == generation for item in shared_target_games)
                         for generation in ("v38", "v40", "v41", "v42", "v43", "v44", "v45", "legacy_absent")},
